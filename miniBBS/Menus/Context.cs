...
namespace miniBBS.Menus
{
    public static class Context
    {
        private static readonly string[] _lines = new[]
        {
            ...
            $"{Constants.Spaceholder}",
            $"Side note: See '{Constants.Inverser}/? msgs{Constants.Inverser}' command on '{Constants.Inverser}/new{Constants.Inverser}' for information about starting a new topic.",
            $"{Constants.Spaceholder}",
            $"For example, let's say you just read message number 1003 and that message shows '{Constants.Inverser}re:987{Constants.Inverser}' (so 1003 is 'in response to' # 987).  If you need context for 1003 " +
            ...
            $"{Constants.Spaceholder}",
            $"This is where the Context command ({Constants.Inverser}/re{Constants.Inverser}) comes in.  It allows you to trace back the thread of conversation by following the breadcrumbs (re: numbers) of each " +
            ...
            $"you can type '{Constants.Inverser}/re{Constants.Inverser}' and message # 987 will show up but your message pointer will not change.",
            $"{Constants.Spaceholder}",
            ...
            $"{Constants.Inverser}/re 0{Constants.Inverser} : Recursively search for the first message in the thread.",
            $"{Constants.Inverser}/re e{Constants.Inverser} : Recursively search for the last message in the thread.",
            $"{Constants.Inverser}/re >{Constants.Inverser} : Advance forward to the next message in the thread.",
            $"{Constants.Inverser}/ra{Constants.Inverser}   : Same as '/re >'.",
